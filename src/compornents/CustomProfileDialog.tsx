  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth slotProps={{ paper: { sx: { borderRadius: 3, minHeight: '600px' } } }}>
      ...
      <Box display="flex" sx={{ height: '600px' }}>
        <Box sx={{ width: 260, flexShrink: 0, bgcolor: 'background.paper', p: 3, borderRight: '1px solid', borderColor: 'divider' }}>
          ...
          <List>
            ...
            <ListItemButton selected={activeTab === 'security'} onClick={() => { setActiveTab('security'); setIsEditingPassword(false); }} sx={{ borderRadius: 1.5, mb: 1 }}>
              <ListItemIcon sx={{ minWidth: 40 }}><SecurityIcon fontSize="small" /></ListItemIcon>
              <ListItemText primary={msg.SECURITY} />
            </ListItemButton>
            <ListItemButton selected={activeTab === 'appearance'} onClick={() => { setActiveTab('appearance'); }} sx={{ borderRadius: 1.5 }}>
              <ListItemIcon sx={{ minWidth: 40 }}><PaletteIcon fontSize="small" /></ListItemIcon>
              <ListItemText primary={msg.APPEARANCE} />
            </ListItemButton>
          </List>
        </Box>
            ...
            <Box>
              ...
              <Paper variant="outlined" sx={{ p: 3, borderRadius: 2 }}>
                <ToggleButtonGroup
                  ...
                  onChange={(e, newMode) => {
                    if (newMode !== null) {
                      colorMode.setThemeMode(newMode);
                    }
                  }}
                ...
                >
                  <ToggleButton value="light" aria-label="light theme">
                    <LightModeIcon sx={{ mr: 1 }} /> {msg.THEME_LIGHT}
                  </ToggleButton>
                  <ToggleButton value="dark" aria-label="dark theme">
                    <DarkModeIcon sx={{ mr: 1 }} /> {msg.THEME_DARK}
                  </ToggleButton>
                </ToggleButtonGroup>
              </Paper>
            </Box>
